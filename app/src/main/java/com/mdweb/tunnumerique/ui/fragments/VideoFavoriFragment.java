 ...
 */
public class VideoFavoriFragment extends Fragment implements View.OnClickListener {
    ...
    private List<News> newsListVideo;
     ...
     */
    public static VideoFavoriFragment newInstance(int typeNews, boolean isFavoris) {
        VideoFavoriFragment fragment = new VideoFavoriFragment();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View rootView = inflater.inflate(R.layout.fragment_video_favori, container, false);
        initView(rootView);
    ...
    public void initView(View rootView) {
        emptyListe = (RelativeLayout) rootView.findViewById(R.id.empty_listeV);
        // get categories list from local json file
        String lang = SessionManager.getInstance().getCurrentLang(requireActivity());
        ...
        } else if (lang.equals(Constant.EN)) {
    ...
    private void loadData() {
        ...
        newsListVideo.add(news);
        newsList.addAll(FavorisDataBase.getInstance(getContext()).getAllNews(SessionManager.getInstance().getCurrentLang(requireActivity())));
        News unNews;
        for (int i = 0; i < newsList.size(); i++) {
            unNews = newsList.get(i);
            if (unNews.getArtOrPubOrVid() == Constant.isVideo)
                newsListVideo.add(unNews);
        }
        articleAdapter.addAll(newsListVideo);
        listSearch.clear();
        listSearch.addAll(newsListVideo);
        listeEmptyTest();
